...
const std::vector<size_t> XYPlayer::play(const Board& board) {
	for (uint x=0; x<board.size(); ++x) {
		for (uint y=0; y<board.size(); ++y) {
			std::vector<size_t> c{x,y};
			if (board[c]=='.') {
				return c;
			}
		}
...
const std::vector<size_t> YXPlayer::play(const Board& board) {
	for (uint y=0; y<board.size(); ++y) {
		for (uint x=0; x<board.size(); ++x) {
			std::vector<size_t> c{x,y};
			if (board[c]=='.') {
 ...
 */
const std::vector<size_t> IllegalPlayer::play(const Board& board) {
...
const std::vector<size_t> ExceptionPlayer::play(const Board& board) {
	throw string("hahaha");
}
